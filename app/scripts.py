...
def load_video(path,file_path):
    ...
    test_results[0].update({'Video_Name': filename})
    test_results[0].update({'Start time': 0})
    test_results[0].update({'End time': duration})
    if test_results[0].get('label') == 'okn':
        test_results[0].update({'Result': 'okn'})
    else:
        test_results.update({'Result': 'neutral gaze'})
